"""
music_emotion_double_sarsa.py  —  Double SARSA for Music Emotion RL
======================================================================
Double SARSA maintains TWO Q-tables (QA, QB) and acts on-policy.
  • Action selection: uses QA + QB combined (sum) for greedy choice.
  • Update:
      - With prob 0.5 → update QA using QB to evaluate ACTUAL next action
      - With prob 0.5 → update QB using QA to evaluate ACTUAL next action
...
Run:
  python music_emotion_double_sarsa.py
"""
...
def reward(state_vec, clip_vec, step_idx: int,
    ...
    if angular_error(next_vec) < TERMINAL_ANGLE_DEG:
        r += terminal_reward_from_clip(step_idx, n_playlist=n_playlist)
...
def compute_mean_clip_vectors(subjects_for_mean: list) -> np.ndarray:
...
class DoubleSARSAAgent:
    ...
    Update rule (randomly choose which table to update):
      If updating QA:
        target  = r + γ · QB(s', a') + φ      ← evaluate ACTUAL next action with QB
    ...
    def update(self, state_idx: int, clip_idx: int,
               r: float, next_state_idx: int, next_clip_idx: int, phi: float):
        """Double SARSA update — use ACTUAL next action."""
        if np.random.rand() < 0.5:
            # Update QA, evaluate ACTUAL action with QB
            q_next  = self.QB[next_state_idx, next_clip_idx]
            td      = r + self.gamma * q_next + phi
            self.QA[state_idx, clip_idx] += self.alpha * (
                td - self.QA[state_idx, clip_idx])
        else:
            # Update QB, evaluate ACTUAL action with QA
            q_next  = self.QA[next_state_idx, next_clip_idx]
            td      = r + self.gamma * q_next + phi
...
def evaluate_subject(agent,
                     test_subject_clips: np.ndarray,
                     n_playlist: int = PLAYLIST_LEN,
                     start_emotion: str = None,
                     bins: int = 20) -> dict:

    test_subject_clips = np.asarray(test_subject_clips, dtype=np.float32)
...
def leave_one_subject_out(subjects: list,
    ...
    print(f"\n  LOSO-DoubleSARSA [{tag}]  alpha={alpha} gamma={gamma} epsilon={epsilon} "
          f"episodes={n_episodes}")
    subjects = prepare_subjects(subjects)

    for test_idx in range(len(subjects)):
        test_subject = subjects[test_idx]
        train_action_vectors = compute_mean_clip_vectors(subjects)
        bins = 20
        n_states = bins * bins

        agent = DoubleSARSAAgent(n_states, len(train_action_vectors), alpha, gamma, epsilon)
        train_agent(agent, train_action_vectors, n_episodes, use_reward_shaping, bins=bins)

        res            = evaluate_subject(agent, test_subject, bins=bins, start_emotion=start_emotion)
...
def grid_search(subjects: list,
    ...
    tag = "w/ shaping" if use_reward_shaping else "w/o shaping"
    print(f"[Grid Search] Double SARSA starting ({tag}) …")
    ...
    for alpha, gamma, epsilon in product(alphas, gammas, epsilons):
        ...
        for val_idx in folds:
            ...
            if not train_idx or not val_idx:
                continue
            train_action_vectors = compute_mean_clip_vectors([subjects[i] for i in train_idx])
            agent = DoubleSARSAAgent(n_states, len(train_action_vectors),
                                         alpha, gamma, epsilon)
            train_agent(agent, train_action_vectors,
                        n_episodes, use_reward_shaping=use_reward_shaping, bins=bins)
            for vi in val_idx:
                fold_errors.append(
                    evaluate_subject(agent, subjects[vi], bins=bins, start_emotion=start_emotion)["final_err"])
...
def print_summary_table(results: list, label: str = "Double SARSA"):
    ...
    print(f"    Angular error < 60 deg  : {len(c60)}/{len(results)}")
...
def plot_angular_error(results: list, save_path: str,
    ...
    ax.errorbar(iters, mean_e, yerr=se, fmt="-o", color="teal",
...
def plot_reward_shaping_comparison(results_with: list, results_without: list,
    ...
    fig, ax = plt.subplots(figsize=(7, 4))
    ax.plot(iters, me(results_with),    "-o",  color="teal",
...
def plot_convergence_comparison(results: list, label: str = "Double SARSA",
                                 save_path: str = "dsarsa_convergence.png"):
    n     = len(results)
...
def plot_emotion_trajectory(result: dict, subject_id: int,
    ...
    ax.plot(xs, ys, "-o", color="teal", lw=1.5, ms=6, zorder=6)
...
def plot_shaping_convergence_bars(results_with: list, results_without: list,
                                  label: str = "Double SARSA",
                                  save_path: str = "dsarsa_shaping_bars.png"):
    """
    ...
    bars1 = ax.bar(x - width/2, with_vals,    width, label="With shaping",
                   color="#1abc9c", edgecolor="k", lw=0.5)
...
def plot_all_subjects_trajectories(results: list, save_path: str,
    ...
    for idx, res in enumerate(results):
        ...
        xs = [EMOTION_CENTERS[e][0] for e in emotions]
        ys = [EMOTION_CENTERS[e][1] for e in emotions]
        ax.plot(xs, ys, "-", color="teal", lw=1.0, zorder=4, alpha=0.8)
...
def main():
    parser = argparse.ArgumentParser(
        description="Double SARSA music emotion RL")
    parser.add_argument("--data_root",   default="/home/sahil/Desktop/Rl_Project_Final/data/Metacsv")
    parser.add_argument("--output_dir",  default="results_double_sarsa")
    ...
    print("=" * 65)
    print("  EEG Music Emotion RL  —  Double SARSA")
    print("=" * 65)
    ...
    # ── Summary tables ──────────────────────────────────────────────
    print_summary_table(results_with,    label="Double SARSA (with shaping)")
    print_summary_table(results_without, label="Double SARSA (without shaping)")
    ...
    plot_angular_error(
        results_with, label="Double SARSA",
        save_path=os.path.join(args.output_dir, "dsarsa_angular_error.png"))

    # 2) Reward shaping comparison (angular error curves)
    plot_reward_shaping_comparison(
        results_with, results_without, label="Double SARSA",
        save_path=os.path.join(args.output_dir, "dsarsa_reward_shaping.png"))

    # 3) Convergence bar chart (with shaping)
    plot_convergence_comparison(
        results_with, label="Double SARSA (with shaping)",
        save_path=os.path.join(args.output_dir, "dsarsa_convergence_criteria.png"))

    # 4) Convergence bar chart (without shaping)
    plot_convergence_comparison(
        results_without, label="Double SARSA (without shaping)",
        save_path=os.path.join(args.output_dir, "dsarsa_convergence_no_shaping.png"))

    # 5) Grouped bar chart: with vs without shaping convergence
    plot_shaping_convergence_bars(
        results_with, results_without, label="Double SARSA",
        save_path=os.path.join(args.output_dir, "dsarsa_shaping_bars.png"))

    # 6) All-subjects emotion wheel trajectories (with shaping)
    plot_all_subjects_trajectories(
        results_with, label="Double SARSA (with shaping)",
        save_path=os.path.join(args.output_dir, "dsarsa_all_trajectories.png"))

    # 7) All-subjects trajectories (without shaping)
    plot_all_subjects_trajectories(
        results_without, label="Double SARSA (without shaping)",
        save_path=os.path.join(args.output_dir, "dsarsa_all_trajectories_no_shaping.png"))
    ...
    if chosen_c and not_converged:
        plot_converged_vs_diverged(
            chosen_c, not_converged[0], label="Double SARSA",
            save_path=os.path.join(args.output_dir, "dsarsa_converged_vs_diverged.png"))

    # 9) Individual emotion trajectory plots
    if converged:
        r = converged[0]
        plot_emotion_trajectory(
            r, r["subject"], label="Double SARSA",
            save_path=os.path.join(args.output_dir,
                                   f"dsarsa_traj_s{r['subject']}_conv.png"))
    if not_converged:
        r = not_converged[0]
        plot_emotion_trajectory(
            r, r["subject"], label="Double SARSA",
            save_path=os.path.join(args.output_dir,
                                   f"dsarsa_traj_s{r['subject']}_div.png"))
