...
def leave_one_subject_out(subjects: list,
    ...
    print(f"\n  LOSO-DoubleSARSA [{tag}] [{dataset_name.upper()}] alpha={alpha} gamma={gamma} epsilon={epsilon} "
    ...
    for test_idx in range(len(subjects)):
        ...
        res            = evaluate_subject(agent, test_subject, bins=bins)
...
def grid_search(subjects: list,
    ...
    tag = "w/ shaping" if use_reward_shaping else "w/o shaping"
    print(f"[Grid Search] Double SARSA starting ({tag}) [{dataset_name.upper()}] …")
    ...
    for alpha, gamma, epsilon in product(alphas, gammas, epsilons):
        ...
        for val_idx in folds:
            ...
            for vi in val_idx:
                fold_errors.append(
                    evaluate_subject(agent, subjects[vi], bins=bins)["final_err"])
...
def main():
    ...
    print("=" * 65)
    print(f"  EEG Music Emotion RL  —  Double SARSA  ({args.dataset.upper()} Dataset)")
    print("=" * 65)
