...
====================================================================
Fixes vs. original:
   1. Convergence = angular-error < 20° (primary) and < 30° (secondary)
   ...
   4. Reports <20° and <30° convergence criteria
...
TARGET_VECTOR         = None
LAMBDA                = -10.0   # reward-shaping penalty (was -100)
...
def leave_one_subject_out(subjects: list,
    ...
    print(f"\n  LOSO [{tag}] [{dataset_name.upper()}] alpha={alpha} gamma={gamma} epsilon={epsilon} "
...
def grid_search(subjects: list,
    ...
    tag = "w/ shaping" if use_reward_shaping else "w/o shaping"
    print(f"[Grid Search] starting ({tag}) [{dataset_name.upper()}] …")
...
def print_summary_table(results: list, label: str = "Q-Learning"):
    ...
    print(f"\n  Convergence summary:")
    print(f"    Angular error < 20°  : {len(c20)}/{len(results)}")
...
def plot_convergence_comparison(results_30: list, results_60: list,
    ...
    """Bar chart: <20° / <30° counts side by side."""
    n        = len(results_30)          # same subjects
    c20      = sum(r["conv_20"]         for r in results_30)
...
def main():
    ...
    print("=" * 65)
    print(f"  EEG Music Emotion RL  —  Q-Learning  ({args.dataset.upper()} Dataset)")
    print("=" * 65)
